<template>
  <main-page title="确认订单" :back="$store.getters.haveRouteFrom">
    <template slot="scroll">
      ...
      <payment-status type="ship" :orderInfo="orderDetail" :status.sync="paymentStatus"></payment-status>
    </template>
  </main-page>
</template>
...
import { getShipOrderDetail } from '@/api'
...
export default {
  ...
  async beforeRouteEnter (to, from, next) {
    ...
    let res = await getShipOrderDetail({orderNo: to.query.orderNo})
  ...
  methods: {
    async getOrderInfo () {
      ...
      let res = await getShipOrderDetail(params)
      this.setData(res)
    },
    setData (res) {
